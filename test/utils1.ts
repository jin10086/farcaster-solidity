...
export function getproof(data: any) {
    ...
    // 处理 hash 字段
    const processedData = JSON.parse(JSON.stringify(data.data)); // 深拷贝以避免修改原始数据
    ...
    }else if (processedData.type === "MESSAGE_TYPE_CAST_ADD" && processedData.castAddBody?.embeds[0]?.castId) {
      processedData.castAddBody.embeds[0].castId.hash = Buffer.from(processedData.castAddBody.embeds[0].castId.hash.slice(2), 'hex');
